...
def extract_and_match_entities(entities: dict) -> dict:
    ...
    if 'publication_type' in entities:
        entities['publication_type'] = word_match(entities['publication_type'], publication_type_patterns)

    if 'activity_level' in entities:
        entities['activity_level'] = word_match(entities['activity_level'], activity_level_patterns)
...
def query_model(q: str) -> ModelResponse:
    ...
    res = requests.post(nlu_url, headers=headers, json={"text": q})
    data = res.json()
